...
const EditEmployee = () => {
  ...
  const { loading, users } = useSelector(
    (state) => state.GetSingleEmployeeReducer
  );
  ...
  //   const [name, setName] = useState(users?.data?.employee?.name);

  //   const [email, setEmail] = useState(users?.data?.user?.name);
  ...
  const handleSubmit = async (e) => {
    ...
    const resData = await dispatch(
      updateEmployeeByAdminAction(id, name, email, password, personName)
    );
